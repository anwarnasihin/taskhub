...
<div class="max-w-3xl mx-auto py-6">
    <div class="bg-white dark:bg-[#18181B] border border-gray-200 dark:border-[#27272A] rounded-2xl shadow-sm p-8">
        ...
        <form action="{{ route('projects.tasks.update', [$project, $task]) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea name="description" rows="4"
                          class="w-full px-4 py-3 bg-white dark:bg-[#202023] border border-gray-300 dark:border-[#27272A] text-gray-900 dark:text-white rounded-xl focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 text-sm transition">{{ old('description', $task->description) }}</textarea>
                @error('description')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                {{-- Tanggal Tenggat --}}
                <div>
                    <label class="block text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-2">Tenggat Waktu (Tanggal)</label>
            ...
            {{-- Lampiran File Baru --}}
            <div>
                <label class="block text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-2">Tambah Lampiran Baru <span class="text-gray-400 font-normal">(opsional · jpg, png, pdf, doc, docx · maks 5MB)</span></label>
