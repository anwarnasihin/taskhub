...
<div class="max-w-4xl mx-auto py-8 sm:px-6 lg:px-8">
    ...
    <div class="mb-8 pl-2">
        <h2 class="text-3xl font-extrabold text-gray-900 tracking-tight">Edit Project</h2>
        <p class="mt-2 text-md text-gray-500">Perbarui informasi project Anda</p>
    </div>
            ...
            <form action="{{ route('projects.update', $project) }}" method="POST">
                ...
                <div class="mb-6">
                    ...
                    <textarea name="description" id="description" rows="3"
                        class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200"
                        placeholder="Deskripsi singkat project ini...">{{ old('description', $project->description) }}</textarea>
                ...
                <div class="mb-10">
                    ...
                    <div class="flex flex-wrap gap-4">
                        @php
                            $colors = ['#3B82F6', '#10B981', '#F59E0B', '#EF4444', '#8B5CF6', '#EC4899', '#F97316'];
                        @endphp

                        @foreach($colors as $color)
                            <label class="cursor-pointer relative flex items-center justify-center">
                                <input type="radio" name="color" value="{{ $color }}" class="peer sr-only" {{ old('color', $project->color) == $color ? 'checked' : '' }}>
